...
function previewFile(card){
    ...
    const printBtn = document.getElementById("printBtn"); // ? ADDED
...
document.addEventListener("DOMContentLoaded", function(){
    ...
    const indexInput = document.getElementById("filterIndex")
    const dropdown = document.getElementById("stationDropdown")

    // ? FIX: no return, guard instead
    if(stationInput && dropdown){
...
function confirmDelete(e, fileId){
    ...
    setTimeout(() => {
        ...
        modal.show();
    }, 150); // allow dropdown to close first
}
...
function loadTrash(){
    ...
    const tableWrapper = document.getElementById("trashTableWrapper"); // ? CORRECT
    ...
    .then(data => {
        ...
        console.log("Trash data:", data); // ?? debug
        ...
        if(!data || data.length === 0){
            ...
            tableWrapper.style.display = "none"; // ? THIS IS THE FIX
